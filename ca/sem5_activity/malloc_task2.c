...
void *realloc(void *ptr, size_t size) {
  ...
  if (asize <= bsize) {
    HEAP_LOCK
    place(ptr, asize);
    g_heap.alloc -= (bsize - GET_SIZE(hdr));
    HEAP_UNLOCK
    return ptr;
  }
  ...
  size_t next_size = GET_SIZE(next_hdr);
  if (!GET_ALLOC(next_hdr) && bsize + next_size >= asize) {
    size_t total = bsize + next_size;
    PUT(hdr, total);
    PUT(PTR_ADD(hdr, total - WSIZE), total);
    place(ptr, asize);
    g_heap.alloc += (GET_SIZE(hdr) - bsize);
    HEAP_UNLOCK
    return ptr;
  }
  HEAP_UNLOCK
